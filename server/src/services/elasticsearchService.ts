class ElasticsearchService {
  ...
  async searchKnowledge(query: string, size: number = 5): Promise<any[]> {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}/${this.indexName}/_search?pretty`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `Elasticsearch HTTP ${response.status}: ${response.statusText}`,
        );
      }
      ...
      if (error.code === "ECONNREFUSED") {
        console.error(
          "🔌 Cannot connect to Elasticsearch. Is it running on",
          this.baseUrl,
          "?",
        );
      }
  ...
  async testConnection(): Promise<boolean> {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          "Elasticsearch connection successful, status:",
          health.status,
        );
        return true;
      } else {
      ...
      if (error.code === "ECONNREFUSED") {
        console.error(
          "🔌 Cannot connect to Elasticsearch. Make sure it's running on",
          this.baseUrl,
        );
      }
